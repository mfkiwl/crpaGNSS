...
M_truth = np.array([[sy*cp,  cr*cy + sr*sy*sp, -sr*cy + cr*sy*sp], 
...
truth_sv_enu = np.array([[ 500,  1500, -2000,  3500, -3000],
                         [-500, -3000,  1500, -2000,  3500], 
                         [6100,  5000,  5500,  4750,  4200]])

for _ in range(1000):
  rpy_noise = rpy_truth + 3*np.random.randn(3)                                #! std of 3 deg     
  ...
  #                     [          -cp*sy,            cp*cy,     sp],
  #                     [sr*cy + cr*sp*sy, sr*sy - cr*sp*cy,  cr*cp]])
  M_noise = np.array([[sy*cp,  cr*cy + sr*sy*sp, -sr*cy + cr*sy*sp], 
                      [cy*cp, -cr*sy + sr*cy*sp,  sr*sy + cr*cy*sp], 
                      [   sp,            -sr*cp,            -cr*cp]])
  ...
  D_loc = (truth_sv_body - truth_user_body[:,None]) / np.linalg.norm(truth_sv_body - truth_user_body[:,None], axis=0)
  ...
  y2.append(R2D * np.arctan(M_hat[0,0] / M_hat[1,0]))
