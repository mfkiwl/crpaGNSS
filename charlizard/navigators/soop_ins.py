...
class SoopIns:
    ...
    @property
    def extract_ecef(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        return self.__r_eb_e, self.__v_eb_e
    ...
    def __init__(self, config: GNSSINSConfig) -> None:
        ...
        att = (config.att.copy() + np.random.randn(3) * 0.5) / R2D  # config.att.copy() / R2D
        ...
        self.__initialize_process_model(config)
    ...
    def measurement_update(
        ...
        # self.__generate_F(0)
        # self.__generate_Q()
        # self.rx_state = self.__F @ self.rx_state
        # self.rx_cov = self.__F @ self.rx_cov @ self.__F.T + self.__Q
        # self.__r_eb_e = self.rx_state[0:3]
    ...
    def mechanize(self, w_ib_b: np.ndarray, f_ib_b: np.ndarray):
        ...
        # gravity, _ = ned2ecefg(self.__r_eb_e, self.__C_e_ned)
    ...
    def __generate_F(self, f_ib_b: np.ndarray):  # , w_ib_b: np.ndarray):
        # self.__F = np.block(
        #     [
        #         [I3, self.T_rcvr * I3, 0.5 * self.T_rcvr**2 * I3, Z32],
        #         [Z33, I3, self.T_rcvr * I3, Z32],
        #         [Z33, Z33, I3, Z32],
        ...
        # self.__F = np.block(
        #     [
        #         [-OMEGA_IE_E, Z33, Z33, Z33, C, Z32],
        #         [f21, -2 * OMEGA_IE_E, f23, C, Z33, Z32],
        #         [Z33, I3, Z33, Z33, Z33, Z32],
        ...
        # self.__F = np.block(
        #     [
        #         [-OMEGA_IE_E, Z33, Z33, Z32],
        #         [f21, -2 * OMEGA_IE_E, f23, Z32],
        #         [Z33, I3, Z33, Z32],
        #         [np.zeros(10), 1],
        #         [np.zeros(11)],
        #     ]
        # )
        # self.__F = expm(self.__F * T)

    #! === Process Noise Covariance ===
    def __generate_Q(self):
        # sigma = 1.0
        # xyz_pp = sigma**2 * self.T_rcvr**5 / 20 * I3
        # xyz_vv = sigma**2 * self.T_rcvr**3 / 3 * I3
        # xyz_aa = sigma**2 * self.T_rcvr * I3
        # xyz_pv = sigma**2 * self.T_rcvr**4 / 8 * I3
        # xyz_pa = sigma**2 * self.T_rcvr**3 / 6 * I3
        # xyz_va = sigma**2 * self.T_rcvr**2 / 2 * I3
        # self.__Q = np.block(
        #     [
        ...
        # self.__Q = np.block(
        #     [
        #         [Srg * I3 * T, Z33, Z33, Z33, Z33, Z32],
        #         [Z33, Sra * I3 * T, Z33, Z33, Z33, Z32],
        #         [Z33, Z33, Z33, Z33, Z33, Z32],
        #         [Z33, Z33, Z33, Sbad * I3 * T, Z33, Z32],
        #         [Z33, Z33, Z33, Z33, Sbgd * I3 * T, Z32],
        #         [Z23, Z23, Z23, Z23, Z23, self.__clk_q],
        #     ]
        # )
        self.__Q = 0.5 * (self.__Q + self.__Q.T)
    ...
    def __generate_H(self, emitter_states: dict, pos: np.ndarray, vel: np.ndarray):
        ...
        # self.__H = np.column_stack((w, u, Z, z1, i1))
    ...
    def __initialize_process_model(self, config: GNSSINSConfig):
        # clock model
        ...
        self.__clk_q = np.array(
            [
                [Sb * self.T + Sd / 3 * self.T**3, Sd / 2 * self.T**2],
                [Sd / 2 * self.T**2, Sd * self.T],
            ]
        )
        self.__clk_q *= SPEED_OF_LIGHT**2 * 1.1
        self.__clk_f = np.array([[1.0, self.T], [0.0, 1.0]])

        # imu model
        if (config.imu_model.casefold() is None) or (config.imu_model.casefold() == "perfect"):
        ...
        # self.rx_state = np.block([self.__r_eb_e, self.__v_eb_e, np.zeros(3), self.__clk_bias, self.__clk_drift])
        # self.rx_cov = np.diag([2.0, 2.0, 2.0, 0.05, 0.05, 0.05, 0.01, 0.01, 0.01, 2.0, 0.1])
        # self.rx_state = np.zeros(23)
        self.rx_state = np.block([np.zeros(15), self.__clk_bias, self.__clk_drift])
        # self.rx_state = np.block([np.zeros(9), self.__clk_bias, self.__clk_drift])
        self.rx_cov = np.diag(
            np.array(
                [
                    0.03,
    ...
    def __initialize_covariance(self, emitter_states: dict):
        ...
        R = np.diag(self.__R)
        k = 0
        while np.any(delta_diag_P > 1e-4) and (k < 1000):
